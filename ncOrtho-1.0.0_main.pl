...
##################################################################################
# Infernal
my $cmbuild     = 'cmbuild';  		# path or command to cmbuild
my $cmsearch    = 'cmsearch';		# path or command to cmsearch
my $cmcalibrate = 'cmcalibrate';	# path or command to cmcalibrate

# BLAST
my $blastn      = 'blastall';		# path or command to blastall
...
##################################################################################
#my $tmp_outpath                 = $outpath.$query_ncrna_name."_".$ukn_genome_name;
#if(not -e $tmp_outpath){
#	system("mkdir $tmp_outpath");
#}
#################################################################################################        
...
###################################################################################
#################### SEARCH UP AND DOWN STREAM SEQUECNE OF NCRNA ##################
###################################################################################
...
##################################################################################################
	...
	### check how much proteins are between up and downstream
	my %count_hash;
	foreach(keys %core_gtf_hash){
		my $species  = $_;
	...
	foreach(keys %core_genome_hash){	
		...
		my $interseq_blast_out = $outfile.".blastout";
		system("$blastn -p blastn -d $outfile -i $ncRNA -o $interseq_blast_out -m 8");     
		if (-z $interseq_blast_out){
			print STDERR "No BLASTN HIT found , skipping $species!\n";
			next;
		}
...
print "STEP 01\n";
system("$tcoffee -special_mode=rcoffee -in $rna_file -output=clustalw_aln > $alignment"); 
...
print "STEP 04\n";
system("$cmcalibrate $covariance_model");
print "STEP 05\n";
system("$cmsearch --tblout $cmsearch_out $covariance_model $ukn_genome");
...
my %result_hash;	# {index} = @(start, stop, chr, strand, score, RBBH_status);
...
########
	...
	system("$blastn -p blastn -d $root_genome -i $uknRNA_file -o $reciproc_blast_out -m 8");
	...
	############# evaluation if predicted positions are correct ##############
	my $rbbh = 0;
	if ($rna_chr eq $predict_rna_chr){
	        if      (($rna_start eq $predict_rna_start) && ($rna_stop eq $predict_rna_stop)){
	                $rbbh = 1;
	        }
	        elsif   (($rna_start eq $predict_rna_start) && ($rna_stop lt $predict_rna_stop)){
	        	$rbbh = 1;
	        }
	        elsif   (($rna_start eq $predict_rna_start) && ($rna_stop gt $predict_rna_stop)){
	                $rbbh = 1;
	        }
	        elsif   (($rna_start lt $predict_rna_start) && ($rna_stop eq $predict_rna_stop)){
	                $rbbh = 1;
	        }
	        elsif   (($rna_start gt $predict_rna_start) && ($rna_stop eq $predict_rna_stop)){
	                $rbbh = 1;
	        }
	        elsif   (($rna_start lt $predict_rna_start) && ($rna_stop lt $predict_rna_stop) && ($rna_stop gt $predict_rna_start)){
	                $rbbh = 1;
	        }
	        elsif   (($rna_start gt $predict_rna_start) && ($rna_stop lt $predict_rna_stop)){
	                $rbbh = 1;
	        }	
	        elsif   (($rna_start lt $predict_rna_start) && ($rna_stop gt $predict_rna_stop)){
	        	$rbbh = 1;
	        }
		elsif   (($rna_start gt $predict_rna_start) && ($rna_stop gt $predict_rna_stop) && ($rna_start lt $predict_rna_stop)){
			$rbbh = 1;
		}
	}
	else{
		...
		my @result_array = ($cmstart, $cmstop, $cmchr, $cmstrand, $cmscore,  'FAIL');
	...
	if ($rbbh == 1){
		#print "@ Reciprocal Best BLAST Hit was successful! - Orthologous sequence for $query_ncrna_name  at: $cmstart - $cmstop - $cmchr\n";
	        my @result_array = ($cmstart, $cmstop, $cmchr, $cmstrand, $cmscore,  'SUCCESS');
                $result_hash{$cm_index}=\@result_array;
...
foreach (keys %result_hash){
	...
	if($tmp_array[5] eq 'SUCCESS'){
		print "#### Potential (CO-)Ortholog found: @tmp_array\n";
		$no_hit_found = 1;
		#### mirko edit #####
		# generate an adaitional outfile if specified by user
		if ($outfile == 1){
...
if ($no_hit_found == 0){
	print "#### No potential orthologs passed the reciprocal best hit criteria\n";
}
############################################################################################################
